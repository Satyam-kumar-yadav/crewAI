...
class Task(BaseModel):
    ...
    def critique(self, agent, task, output, llm):
        """
        Critiques the output generated by an LLM model for a given task.

        Args:
        - agent (Agent): The agent assigned to the task.
        - task (Task): The task description and context.
        - output (str): The output generated by the LLM model.
        - llm (ChatOpenAI): An instance of the LLM model used for critique analysis.

        Returns:
        - str: A critique highlighting logical or factual inaccuracies in the output.
        """

        critic_prompt_template = """
        {agent_backstory}. You are a meticulous critic with a sharp eye for errors.
        A task has been assigned to an LLM model which has generated an output based on the task description.
        Your role is to identify any errors in the output provided by the model.

        Strictly focus on pointing out logical or factual inaccuracies. Avoid grammatical rephrasing or paraphrasing.
        Do not provide your own rewritten version of the output, only critique the existing output.
    ...
    def validate(self, task, critique, output, llm):
        """
        Validates a critique on task output to determine if significant changes are required.

        Args:
        - task (Task): The task description and context.
        - critique (str): The critique of the output highlighting errors.
        - output (str): The output generated by the LLM model.
        - llm (ChatOpenAI): An instance of the LLM model used for validation.

        Returns:
        - str: 'True' if significant changes are required, 'False' otherwise.
        """

        validate_prompt_template = """
        As a context analyzer, your task is to assess critiques on task outputs and determine if significant changes are required.

        Task Description: {task_description}
        Output: {output}
        Critique: {critique}

        Your goal is to decide if the critique suggests "significant changes required" or similar phrases.

        - Print "True" if the critique explicitly demands significant changes without preamble.
        - Print "True" if the critique suggests tangential changes.
        - Print "False" if the critique mentions only minor inaccuracies or adjustments.
        - Print "False" otherwise.

        Your output should be only 'True' or 'False'.
        """
    ...
    def improve(self, task, output, critique, llm):
        """
        Improves the task output based on a critique.

        Args:
        - task (Task): The task description and context.
        - output (str): The output generated by the LLM model.
        - critique (str): The critique suggesting improvements.
        - llm (ChatOpenAI): An instance of the LLM model used for output improvement.

        Returns:
        - str: The improved output based on the critique.
        """
    
        improve_prompt_template = """
        You are an adept assistant skilled in analyzing critiques and improving task outputs.
        Your task is to thoroughly understand the task, its output, and the provided critique, then rewrite the output based on the critique.

        Avoid starting with "Here is my modification" or similar phrases. Do not add any preamble or explanations.
        Simply rewrite the output as per the critique, maintaining the specified format if mentioned in the task description.
    ...
    def _execute(self, agent, task, context, tools, rci=True, rci_depth=1):
        """
        Executes a task using an agent, performs critique, validation, and potential improvement.

        Args:
        - agent (Agent): The agent responsible for executing the task.
        - task (Task): The task description and context.
        - context (dict): Additional context or parameters for task execution.
        - tools (dict): Tools or resources required for task execution.
        - rci (bool): Flag to enable Request-Clarification-Integration (RCI) process.
        - rci_depth (int): Depth of RCI iterations.

        Returns:
        - str: The final output after task execution and potential improvement.
        """
        ...
        llm = ChatOpenAI(
            model = "mistral-small",
            base_url = "https://api.mistral.ai/v1") ### LLM Calling
